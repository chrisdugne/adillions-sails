...
# Pass through JSHint validator
for * .js files in the commit
pass = true
printf "\e[4mRunning JShint validation...\e[0m\n"
for file in $ {
  files
};
do
  if [["$file" == * .js]]; then
if [["$file" == * /assets/scripts
    /* ]]; then
...
if ! $pass; then
  ...
  printf "Run 'grunt jshint' and fix the errors.\n"
  exit 1
fi
