...
window.adillions.bus = _.extend({}, Backbone.Events);

/**
 * Pulls the language from the URL, expecting it to be in the format adillions.com/fr
...
// Listen for new CSRF event and update meta tag
window.adillions.bus.on('csrf:new', function () {
  $.get('/csrfToken').then(function (res) {
    if (res && res.status === 'SUCCESS') {
      meta.attr('content', res.data._csrf);
    }
  });
});
